...
function getClient(): SupabaseClient<Database> {
  ...
  // Same public project URL as the browser client; fall back to the constant so
  // a missing/mis-typed env can't misconfigure the server client either. The
  // service-role KEY stays env-only — it is a secret and must never be hardcoded.
  const url = process.env.NEXT_PUBLIC_SUPABASE_URL || 'https://tjlyrgvdqgyafamiwyus.supabase.co'
